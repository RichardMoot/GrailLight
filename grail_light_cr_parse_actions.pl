...
main :-
	current_prolog_flag(os_argv, Argv),
        append(_, [A|Av], Argv),
	file_base_name(A, 'grail_light_cr_parse_actions.pl'),
	!,
        main(Av).
...
compute_chart_proof(Index0, rule(Rule,Pros,tuple(Formula,Sem,stacks(Stack1,Stack2,Stack3,Stack4)),Prems)) :-
	get_stored(Index0, Index, Idf, _L, _R, Formula, data(Pros0,Sem,_,_,Stack1,Stack2,Stack3,Stack4)),
	justification(Index, Just),
	Just =.. [Rule|Args0],
...
proof_axioms(rule(axiom,Pros,tuple(Formula,word(N),_),[]), Ax0, Ax) :-
	!,
...
match_antecedent([], [], []).
match_antecedent([I|Is], [item(Formula,L,R,Data0)|As], [rule(_Rule,_Pros,tuple(Formula,Sem,stacks(S1,S2,S3,S4)),_)|Rs]) :-
	get_stored(I, _, _, L, R, Formula, Data0),
	Data0 = data(_, Sem, _, _, S1, S2, S3, S4),
