...
main :-
	current_prolog_flag(os_argv, Argv),
        append(_, [A|Av], Argv),
	file_base_name(A, 'grail_light_eval.pl'),
	!,
        main(Av).
...
find_gaps(List, Max) :-
	find_gaps(1, Max, [], List).
	

find_gaps(G, Max, List, List) :-
	G  >= Max,
	!.
find_gaps(G0, Max, List0, List) :-
	(
	  (stored(_, _, I, J, _, _), J - I > 2, I < G0, G0 < J)
	->
	   List1 = List0
	;
	   List1 = [G0|List0]
	...
	G1 is G0 + 1,
	find_gaps(G1, Max, List1, List).
	
output_best_span(N) :-
	sentence_length(Max),
	find_gaps(Gaps),
	output_best_spans(Gaps, 0, Max, ProsList),
	print_proof(N, rule(axiom, t(ProsList), lit(let)-true, []), pl_proof).


output_best_spans([], Min, Max, [Pros]) :-
	best_prob_index(Min, Max, Index),
	compute_chart_proof(Index, rule(_,Pros,_,_)).
output_best_spans([G|Gs], Min, Max, [Pros|Ps]) :-
	best_prob_index(Min, G, Index),
	compute_chart_proof(Index, rule(_,Pros,_,_)),
	output_best_spans(Gs, G, Max, Ps).

best_prob_index(I, J, Index) :-
	findall(Prob-Index, stored(Index, _, I, J, _, data(_,_,Prob,_,_,_,_,_)), List),
	List = [Max0-Index0|Rest],
	find_max_index(Rest, Max0, Index0, Index).

find_max_index([], _, Index, Index).
find_max_index([Max1-Index1|Rest], Max0, Index0, Index) :-
	(
		Max1 > Max0
	->
	        Index2 = Index1,
	...
	find_max_index(Rest, Max2, Index2, Index).
