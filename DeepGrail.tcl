...
set berkeley_jar       [file normalize "../BerkeleyParser/BerkeleyParser-1.7.jar"]
set berkeley_grammar   [file normalize "../BerkeleyParser/fra_sm5.gr"]
...
set berkeley_parser_cmd "/usr/bin/java -jar $berkeley_jar -gr $berkeley_grammar  "
set bootstrap "berkeley"
set pytorch_cmd "/Users/moot/python/pytorch39/bin/python"
#set pdflatex_cmd "/Library/TeX/texbin/pdflatex"
set pdflatex_cmd "/usr/local/texlive/2025/bin/universal-darwin/pdflatex"
...
proc latex_proofs {} {
    ...
    global tmp_dir pdflatex_cmd

    set cur_dir [pwd]
    cd $tmp_dir
    if [catch {exec $pdflatex_cmd latex_proofs.tex} pdfErrmsg] {
	   puts stderr $pdfErrmsg
    }
    if [catch {exec open latex_proofs.pdf} prevErrmsg] {
	   puts stderr $prevErrmsg
    }
    cd $cur_dir
...
proc update_menus {} {
	...
	.mb.options.bparser entryconfigure 0 -state normal
	.mb.options.bparser entryconfigure 1 -state normal
	.mb.options.bparser entryconfigure 2 -state normal
    } else {
	...
	.mb.options.bparser entryconfigure 0 -state disabled
	.mb.options.bparser entryconfigure 1 -state disabled
	.mb.options.bparser entryconfigure 2 -state disabled
    }
}
...
#	puts stderr ""
	...
	if {[catch {exec $pytorch_cmd predict.py} super_msg]} {
	    puts stderr $super_msg
	}
	cd $saveDir
    } else {
    ...
    # create parser input file
    if {[string equal $lemmatiser "lefff"]} {
	set parser_file [open "$tmp_dir/parser_nolem.pl" w]
    } else {
	set parser_file [open "$tmp_dir/parser.pl" w]
    }
    
    puts -nonewline $parser_file "% Automatically generated by DeepGrail.tcl on "
    puts $parser_file [clock format [clock seconds] -format "%a %d %b %Y"]
    puts $parser_file ""

    # read the supertagger output and translate it into parser input
    set sentcounter -1
	...
	set tcomma ""
	puts $parser_file "sent([expr $sentcounter+1], A) :-"
	puts -nonewline $parser_file "     prob_parse(\["

	
	if {$sentcounter > 0} {
	    .c delete all
	    printwords 190 [lindex $s_list $sentcounter]
	    printpos 160 [lindex $p_list $sentcounter]
	    update idletasks
	...
	set list_counter -1
	foreach i $list1 {
	    # graphical output to supertag window
	    incr list_counter
	    set il [split $i "|"]
...
#	    set lemma [backslash_interpunction [lindex $lem_list $list_counter]]
	    ...
	    puts -nonewline $parser_file "       $itemsi'$word', [string tolower $pos], '[lindex $lemma 0]', \["
	    set tcomma ","
	    foreach j $nlist {
		puts stderr $j
		lappend flist $j
    ...
    # Lefff lemmatisation
    if {[string equal $lemmatiser "lefff"]} {
	set comment "Lemmatising"
	printcurrentcomment
	update idletasks
	puts -nonewline stderr "Lemmatising (Lefff) "
	if {[catch {exec /Users/moot/checkout/GrailLight/lefff.pl "$tmp_dir/parser_nolem.pl"} errMsg]} {
            puts stderr "error!"
	    puts stderr $errMsg
	   } else {
	    puts stderr "done!"
    ...
    puts -nonewline $f3 "# Automatically generated by DeepGrail.tcl on "
...
proc bootstrap_parser {tmp_dir} {

    global stanford_parser_cmd stanford_parser_length berkeley_parser_cmd bootstrap grail_prefix

    set cur_dir [pwd]
    cd $tmp_dir
    
    if {[string equal $bootstrap "stanford"] && [file executable $stanford_parser_cmd]} { 
	# use Stanford parser to compute constituent structure
	# parse input sentences
	if {[catch {exec $stanford_parser_cmd $stanford_parser_length $tmp_dir/input.txt} bparse_msg]} {
	...
	if {[catch {exec /bin/tcsh -c "$berkeley_parser_cmd < $tmp_dir/input.txt > $tmp_dir/input.berkeley"} bparse_msg]} {
	    puts stderr $bparse_msg
	}
	# if a parse has been found, convert it to crosses declarations and add these to the GrailLight input
	if {[file exists $tmp_dir/input.berkeley]} {
...
.mb.options.bparser add radio -label "Berkeley Parser" -variable bootstrap -value berkeley -command {
    update_menus
}
.mb.options.bparser add radio -label "None" -variable bootstrap -value none -command {
    update_menus
}
menu .mb.options.parser
.mb.options.parser add radio -label "Grail + Chart" -variable grail_parse -value chart_pos_lemma -command {
    update_menus
}
